<?php
/**
 * Weekly Horoscope Module Database Layer
 * 
...
function cv_get_weekly_horoscopes($args = array()) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
    ...
    if (!empty($args['week_start'])) {
        $where[] = 'week_start = %s';
        $params[] = sanitize_text_field($args['week_start']);
    }
    ...
    $order_by = in_array($args['order_by'], array('id', 'week_start', 'zodiac_sign', 'status')) ? $args['order_by'] : 'week_start';
...
function cv_get_weekly_horoscope($id) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
    return $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE id = %d", (int) $id));
}

function cv_count_weekly_horoscopes($args = array()) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
    $where = array('1=1');
    $params = array();
...
function cv_insert_weekly_horoscope($data) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
    $formats = array(
        'week_start'     => '%s',
...
function cv_update_weekly_horoscope($id, $data) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
    $formats = array(
        'week_start'     => '%s',
...
function cv_delete_weekly_horoscope($id) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
    return $wpdb->delete($table, array('id' => (int) $id), array('%d')) !== false;
}

function cv_delete_bulk_weekly_horoscopes($ids) {
    global $wpdb;
    $table = $wpdb->prefix . 'cv_weekly_horoscope';
